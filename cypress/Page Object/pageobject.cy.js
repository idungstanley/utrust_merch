export default class Utrust{
    ...
    click_payment(){
        return cy.get("ul[role='menubar'] li[role='menuitem'] a[href='/payments/orders']").click({force:true})
    }
    click_invoice(){
        return cy.get(':nth-child(2) > .Tabs_tab__3Yjya').click({force:true})
    }
    add_newInvoice(){
        return cy.get("div .Payments_newInvoiceDesktop__oV8L2 button[name='button']").click({force:true})
    }
    enter_name(name){
