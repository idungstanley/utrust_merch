export default class Utrust {
  ...
  validate_email(serviceId,emailAddress){
    return cy.mailosaurGetMessage(serviceId, {
      sentTo: emailAddress,
    }).then((email) => {
  ...
  logout() {
    return cy
      .get(".Sidebar_signOutWrapper__2QOPL div a[href='/sign-out']")
      .click();
  }

  select_details() {
    return cy.get("#cryptocurrencies-list").click();
  }

  validate_amount(amount) {
    return cy
      .get(".Details_total__2yYcA .Details_fiat__2oK6C")
      .should("contain", amount);
  }

  validate_total_amount(amount) {
    return cy
      .get(".Details_column__3cWJk .Details_totalFiat__1Dk90")
      .should("contain", amount);
  }
  ...
  validate_store_name(store) {
    return cy
      .get(".Details_inline__1DFay .Details_storeName__3iENh")
